...
sudo docker run --network=RainMakerNetwork --ip=10.10.10.2 --name=WebRole -d -p 9000:5000 flask-webrole

#create WorkerRole image and container
sudo docker build -t flask-workerrole:latest ./WorkerRole/
sudo docker run --network=RainMakerNetwork --ip=10.10.10.3 --name=WorkerRole -d -p 9001:5000 flask-workerrole
